...
const deal = (cardIdx, toPlayer, faceDown) => {
	...
	newCardImg.setAttribute('src', createCardSrc(card, faceDown));
	newCardImg.setAttribute('id', cardIdx);
	newCardImg.setAttribute(
		'class',
		'playing-card col-5 col-sm-4 col-md-3 col-xl-2'
	);

	if (toPlayer) {
		playerCardsDiv.appendChild(newCardImg);
		playerHand.push(card);
	} else {
		dealerCardsDiv.appendChild(newCardImg);
...
const flipCard = (cardIndex) => {
	const flippedCard = document.getElementById(cardIndex);
	const card = playingCards[cardIndex];
	flippedCard.setAttribute('src', createCardSrc(card, false));
};

const dealerTurn = async (flipDealerCard = true) => {
	playerAction = false;
	buttonsDiv.setAttribute('class', 'd-none');
